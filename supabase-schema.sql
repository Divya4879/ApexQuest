...
-- Insert admin and moderator users
INSERT INTO users (auth0_id, email, name, role) VALUES
('admin-auth0-id', 'admin@apexquest.com', 'Admin User', 'admin'),
('mod-auth0-id', 'mod@apexquest.com', 'Moderator User', 'moderator');
...
CREATE POLICY "Users can update own profile" ON users FOR UPDATE USING (auth0_id = current_setting('app.current_user_auth0_id'));

-- Posts are visible to channel members
CREATE POLICY "Users can view posts in their channels" ON posts FOR SELECT USING (
  channel_id IN (
    SELECT channel_id FROM user_channels 
    WHERE user_id = (SELECT id FROM users WHERE auth0_id = current_setting('app.current_user_auth0_id'))
  )
);

-- Users can create posts in their channels
CREATE POLICY "Users can create posts in their channels" ON posts FOR INSERT WITH CHECK (
  user_id = (SELECT id FROM users WHERE auth0_id = current_setting('app.current_user_auth0_id'))
  AND channel_id IN (
    SELECT channel_id FROM user_channels 
    WHERE user_id = (SELECT id FROM users WHERE auth0_id = current_setting('app.current_user_auth0_id'))
  )
...
-- Similar policies for replies, likes, etc.
CREATE POLICY "Users can view replies on accessible posts" ON replies FOR SELECT USING (
  post_id IN (
    SELECT id FROM posts WHERE channel_id IN (
      SELECT channel_id FROM user_channels 
      WHERE user_id = (SELECT id FROM users WHERE auth0_id = current_setting('app.current_user_auth0_id'))
    )
  )
